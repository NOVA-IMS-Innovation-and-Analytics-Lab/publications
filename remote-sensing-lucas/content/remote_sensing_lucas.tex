...
\abstract{The automatic production of Land Use/Land Cover maps continues to be a
...
automatic classifiers, able to produce accurate maps, can have a significant
impact on the way we manage and optimize natural resources. The difficulty in
...
common and difficult problem in remote sensing that affects the quality of
classification results. We propose Geometric-SMOTE, a novel oversampling method,
as a tool for addressing the imbalanced learning problem in remote sensing.
Geometric-SMOTE is a sophisticated oversampling algorithm which increases the
quality of the generated instances over previous methods, such as Synthetic
Minority Oversampling TEchnique. The performance of Geometric-SMOTE, in the
LUCAS dataset, is compared to other oversamplers using a variety of classifiers.
The results show that Geometric-SMOTE significantly outperforms all the other
...
European Commission (Eurostat) \cite{LUCAS2015}. LUCAS surveys are carried out
every 3 year since 2006 and are freely accessible.  For this statistical
sampling survey 2 km regular grid is implemented and over 1,000,000 points are
...
\ref{fig:studyarea}. The area contains all 8 main land cover types present in
LUCAS 2015.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{../analysis/gsmote_smote}
	\caption{Study area and ground-truth data.}
	\label{fig:studyarea}
\end{figure}

\subsection{Remote sensing data}

The remotely sensed data includes 8 images from Landsat 8ETM+ multi-spectral
...
mode is Descending.  Data was preprocessed in order to remove pixels with cloud
cover. Only bands 2, 3, 4, 5, 6 and 7 are used from each image. Accordingly,
each reference point from LUCAS dataset has 48 features.
...
resolution photo interpretation  [reference: LUCAS2015 C1 instruction]. If the
land cover is homogeneous in the sampling point area, then the land cover type
within the defined area is assigned to the LUCAS point. When the land cover is
not homogeneous, systematic observation of land cover is carried out  in the
...
cover type in Landsat pixel coverage area of 30x30 m. Further, we assume that
classifiers are capable to overcoming the noise caused by pixels having mixed
land cover representation, if such pixels are still available in the dataset.
