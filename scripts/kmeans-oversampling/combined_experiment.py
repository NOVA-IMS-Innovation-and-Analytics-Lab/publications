...
results_path = join(dirname(__file__), '..', 'data', 'results', 'kmeans-oversampling', 'smote')

# Oversamplers and classifiers
oversamplers = [
    ('NO OVERSAMPLING', None),
    ('K-MEANS RANDOM OVERSAMPLING', RandomOverSampler(clusterer=KMeans(random_state=0), distributor=DensityDistributor(), random_state=1), {
        'clusterer__n_clusters': [0.0, 0.1, 0.25, 0.5, 0.75, 1.0],
        'distributor__distances_exponent': [0, 1, 2],
        'distributor__filtering_threshold': [0.5, 1.0]
        }
    ),
    ('K-MEANS SMOTE', SMOTE(clusterer=KMeans(random_state=0), distributor=DensityDistributor(), random_state=2), {
        'k_neighbors': [3, 4, 5],
        'clusterer__n_clusters': [0.0, 0.1, 0.25, 0.5, 0.75, 1.0],
        'distributor__distances_exponent': [0, 1, 2],
        'distributor__filtering_threshold': [0.5, 1.0]
    ...
    ('K-MEANS BORDERLINE-SMOTE', SMOTE(clusterer=KMeans(random_state=0), distributor=DensityDistributor(), random_state=3, kind='borderline1'), {
        'k_neighbors': [3, 4, 5],
        'm_neighbors': [6, 8, 10],
        'clusterer__n_clusters': [0.0, 0.1, 0.25, 0.5, 0.75, 1.0],
        'distributor__distances_exponent': [0, 1, 2],
    ...
    ('K-MEANS G-SMOTE', GeometricSMOTE(clusterer=KMeans(random_state=0, n_init=1), distributor=DensityDistributor(), random_state=4), {
...
classifiers = [
    ...
    ('DT', DecisionTreeClassifier(random_state=5), {'max_depth': [3, 6]}),
    ('GBC', GradientBoostingClassifier(random_state=6), {'max_depth': [3, 6], 'n_estimators': [50, 100]})
]
